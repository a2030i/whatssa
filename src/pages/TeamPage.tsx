    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-6 max-w-[1100px] mx-auto" dir="rtl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 animate-fade-in">
        {[
          ...
          { value: profiles.filter((m) => m.is_online || (m.last_seen_at && Date.now() - new Date(m.last_seen_at).getTime() < 2.5 * 60 * 1000)).length, label: "متصلون الآن", color: "text-success", iconBg: "bg-success/10" },((m) => m.is_online || (m.last_seen_at && Date.now() - new Date(m.last_seen_at).getTime() < 2.5 * 60 * 1000)).length, label: "متصلون الآن", color: "text-success", iconBg: "bg-success/10" },, color: "text-success", iconBg: "bg-success/10" },
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {teams.map((team) => {
            ...
            const onlineCount = teamMembers.filter((m) => m.is_online || (m.last_seen_at && Date.now() - new Date(m.last_seen_at).getTime() < 2.5 * 60 * 1000)).length; = teamMembers.filter((m) => m.is_online || (m.last_seen_at && Date.now() - new Date(m.last_seen_at).getTime() < 2.5 * 60 * 1000)).length;
