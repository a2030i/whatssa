...
import { Bot, Plus, Trash2, Edit, Save, Loader2, MessageCircle, Tag, Users, ScrollText, Mail, MessageSquare as MsgSq } from "lucide-react";
...
import EmailAutomationRules from "@/components/integrations/EmailAutomationRules";
...
const AutomationPage = () => {
  ...
  const loadAgents = async () => {
    if (!orgId) return;
    const { data } = await supabase.from("profiles").select("id, full_name").eq("org_id", orgId).eq("is_active", true);
  ...
  return (
    <div className="p-3 md:p-6 space-y-5 max-w-[900px] mx-auto" dir="rtl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">قواعد تلقائية لتوجيه المحادثات والإيميلات</p>
      </div>
      ...
      <Tabs defaultValue="whatsapp" className="w-full">
        ...
        <TabsContent value="whatsapp" className="space-y-5 mt-4">
          <div className="flex items-center justify-between">
            <p className="text-xs text-muted-foreground">قواعد الرد والوسوم والإسناد بناءً على الكلمات المفتاحية</p>
