...
const AdminOverview = () => {
  ...
  const load = async () => {
    try {
      const safeRpc = (p: PromiseLike<any>) => Promise.resolve(p).catch(() => ({ data: null }));
      const [statsRes, hourlyRes, topOrgsRes, profiles, orgs, plans, wallets, transactions, usage, infraRes] = await Promise.all([
        safeRpc(supabase.rpc("admin_get_system_stats")),
        safeRpc(supabase.rpc("admin_get_hourly_messages", { _date: new Date().toISOString().slice(0, 10) })),
        safeRpc(supabase.rpc("admin_get_top_orgs_usage", { _limit: 10 })),
        ...
        safeRpc(supabase.rpc("admin_get_infra_status")),
      ]);
