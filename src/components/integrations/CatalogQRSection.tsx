...
export const CatalogSection = () => {
  ...
  const loadProducts = async (catalog: Catalog) => {
    ...
    const { data, error } = await invokeCloud("whatsapp-catalog", { body: { action: "list_products", catalog_id: catalog.id } });
...
export const QRCodeSection = () => {
  ...
  const loadQR = async () => {
    setIsLoading(true);
    const { data } = await invokeCloud("whatsapp-catalog", { body: { action: "list_qr" } });
  ...
  const deleteQR = async (qrId: string) => {
    const { data } = await invokeCloud("whatsapp-catalog", { body: { action: "delete_qr", qr_id: qrId } });
