...
const CustomerInfoPanel = ({ conversation, onUpdateNotes, onAssignAgent, onAssignTeam, isMobileSheet }: CustomerInfoPanelProps) => {
  ...
  const SectionHeader = ({ title, icon: Icon, sectionKey }: { title: string; icon: any; sectionKey: keyof typeof sections }) => (
    <button onClick={() => toggleSection(sectionKey)} className="w-full flex items-center justify-between py-2.5 group">
      <p className="text-[11px] font-semibold text-muted-foreground/80 uppercase tracking-wider flex items-center gap-1.5">
        <Icon className="w-3.5 h-3.5" /> {title}
      </p>
      {sections[sectionKey] ? <ChevronUp className="w-3.5 h-3.5 text-muted-foreground/50" /> : <ChevronDown className="w-3.5 h-3.5 text-muted-foreground/50" />}
    </button>
  );
  ...
  return (
    <div className={isMobileSheet ? "flex flex-col overflow-y-auto bg-card" : "w-[300px] h-full bg-card/80 backdrop-blur-sm hidden xl:flex flex-col overflow-y-auto scrollbar-thin"}>
      ...
      <div className="p-5 border-b border-border/30 text-center">
        <div className="relative inline-block">
          {groupPicture ? (
            <img src={groupPicture} alt={conversation.customerName} className="w-16 h-16 rounded-2xl object-cover mx-auto mb-3 shadow-sm" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; (e.target as HTMLImageElement).nextElementSibling?.classList.remove("hidden"); }} />
          ) : null}
          <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center text-xl font-bold text-primary mx-auto mb-3 ${groupPicture ? "hidden" : ""}`}>
        ...
        <h3 className="font-bold text-sm text-foreground">{conversation.customerName}</h3>
        <p className="text-[11px] text-muted-foreground mt-0.5">{isGroup ? `${groupParticipants.length} عضو` : (conversation.lastSeen || "غير متصل")}</p>
        {customer && !isGroup && (
          <Badge variant="outline" className="text-[10px] mt-2 gap-1 rounded-full">
