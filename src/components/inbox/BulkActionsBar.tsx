...
import { X, XCircle, UserPlus, Tag, CheckCircle2, Loader2, Archive } from "lucide-react";
...
const BulkActionsBar = ({ selectedIds, onClear, onDone }: BulkActionsBarProps) => {
  ...
  const handleBulkClose = async () => {
    if (!window.confirm(`هل تريد إغلاق ${count} محادثة؟`)) return;
    setLoading(true);
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-primary/5 border-b border-primary/20 animate-fade-in">
      ...
      <div className="flex items-center gap-1 flex-1 overflow-x-auto">
        <Button variant="ghost" size="sm" className="text-xs gap-1 shrink-0 h-7" onClick={handleBulkClose} disabled={loading}>
